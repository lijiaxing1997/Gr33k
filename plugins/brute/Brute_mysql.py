...
class Brute_mysql(QThread):
    ...
    def run(self):
        ...
        self.brute_mysql(ip_list,username_file,password_file,thread)
        self.login = []
            ...
            if plugins.config.brute_wait == 0:
                ...
                try:
                    ...
                    self.str_signal[str, str].emit(
                        '[+] ' + time.strftime('%H:%M:%S', time.localtime()) + ' mysql成功 %s -- >%s : %s' % (
                    ...
                    self.str_signal[str, str].emit(
                        '[-] ' + time.strftime('%H:%M:%S', time.localtime()) + ' mysql错误 %s --> %s : %s' % (
                            self.host, self.user, password), 'red')
            else:
                time.sleep(1)


    def brute_mysql(self,ip, user_dict, pass_dict, thread_count):  # ip地址(列表)，账号文件，密码文件，线程数
        thread = []
        ...
        for i in range(thread_count):
            f = threading.Thread(target=self.thread_mysql)
            f.start()
            thread.append(f)
        for i in thread:
            i.join()
    ...
    def connect(self,host,port,user,password):
        s = pymysql.connect(host=host,user=user,password=password,database='mysql',port=int(port),charset="utf8")
        s.close()
